"""
酒馆战棋联赛网站（淘汰赛版）— 入口文件
模块拆分：db / auth / cleanup / data / routes_* / sse / routes_tournament
"""
...
from routes_tournament import tournament_bp
...
def _background_cleanup_with_enroll():
    ...
    while True:
        try:
            from cleanup import cleanup_stale_games, cleanup_partial_matches, cleanup_stale_queues, cleanup_expired_bind_codes
            cleanup_stale_games()
...
_cleanup_thread = threading.Thread(target=_background_cleanup_with_enroll, daemon=True)
