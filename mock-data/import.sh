...
# 清空旧数据
echo "清空旧集合..."
mongosh "$DB" --eval "db.league_players.drop(); db.league_matches.drop();" --quiet
...
mongosh "$DB" --eval '
  ...
  db.league_matches.createIndex({ "players.placement": 1 });
' --quiet
...
mongosh "$DB" --eval '
  ...
  print("league_matches (completed): " + db.league_matches.countDocuments({ endedAt: { $ne: null } }) + " 条");
  print("league_matches (active):    " + db.league_matches.countDocuments({ endedAt: null }) + " 条");
' --quiet
