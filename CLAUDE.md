...
- **SSE 连接必须在页面跳转时关闭**：所有 EventSource 需注册到 `window.__sse_list`，`base.html` 的 `beforeunload` 统一清理。不关闭会导致服务端僵尸 generator 堆积，每 2 秒查一次 MongoDB，反复跳转页面后 CPU 飙升
- **MongoDB 索引**：`db.py` 的 `_ensure_indexes()` 在首次连接时自动创建。新增查询字段需同步加索引，避免全集合扫描
- **build_bracket_data() 缓存**：5 秒 TTL，对阵图页面 SSE 每 2 秒轮询。waiting 组数据直接复用 `all_rankings`，不要逐组跑聚合
