...
### v0.17.13 (2026-05-02) — SSE 增量推送，对阵图带宽降 98%
- **SSE delta 推送**: 对阵图 SSE 从全量推送改为增量推送（delta），仅发送变化的组数据
- **首次全量 + 后续 delta**: 新连接收到完整数据，后续只推变化的 group（~3 KB vs 原 332 KB）
- **断线重连补发**: 支持 `Last-Event-ID`，客户端重连时自动补发缺失的 delta
- **环形缓冲**: 服务端保留最近 50 条 delta，seq 过旧时自动降级为全量推送

### v0.17.12 (2026-05-02) — 对阵图 HTTP 缓存
- 对阵图页面和 API 添加 ETag + Cache-Control（5s），减少重复请求
