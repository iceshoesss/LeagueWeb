...
## 更新日志
### v0.18.11 (2026-05-03) — 内存泄漏修复
- **SSE 生成器断连修复**: 客户端断开时 `BrokenPipeError`/`ConnectionResetError` 被 `except Exception` 吞掉导致 greenlet 堆积，改为立即 break 释放
...
- **SSE CacheEntry 清理**: 移除未使用的 `_cond` 属性
...
- PR 仅构建验证，不推送镜像
- 每周日自动清理 7 天前的 nightly 镜像，保留 `:nightly` 最新版

### v0.18.3 (2026-04-30) — 淘汰赛决赛轮修复
- 对阵图最后 3 轮（四分之一决赛、半决赛、决赛）始终不折叠
- 决赛组完成后不再错误创建下一轮分组
- 管理员手动晋级同样禁止决赛组创建下一轮
- 修复导出晋级名单未遵守赛事晋级规则的问题

### v0.18.2 (2026-04-29) — 插件日志优化
- `upload-rating` 接口日志新增 playerId，便于排查玩家记录问题
