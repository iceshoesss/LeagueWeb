...
@admin_bp.route("/api/admin/group/<group_id>/edit-record", methods=["PUT"])
def api_admin_edit_record(group_id):
    """纠错：修改已补录的对局结果"""
    admin_tag = _admin_required()
    if not admin_tag:
        return jsonify({"error": "需要管理员权限"}), 403
    ...
    if not match:
        return jsonify({"error": "该组没有对局记录"}), 404

    # 校验排名
    match_players = match.get("players", [])
    n = len(match_players)
    placement_vals = [int(v) for v in placements.values()]
    ...
    now_str = datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
    update_ops = {}
    for i, p in enumerate(match_players):
        lo = str(p.get("accountIdLo", ""))
        if lo in placements:
            new_placement = int(placements[lo])
            new_points = 9 if new_placement == 1 else max(1, 9 - new_placement)
            update_ops[f"players.{i}.placement"] = new_placement
            update_ops[f"players.{i}.points"] = new_points

    if update_ops:
        db.league_matches.update_one({"_id": match["_id"]}, {"$set": update_ops})
        log.info(f"[edit-record] 管理员 {admin_tag} 修改 R{group.get('round')}G{group.get('groupIndex')} 结果: {len(update_ops)//2} 人")

    return jsonify({"ok": True, "matchUuid": match.get("gameUuid")})
